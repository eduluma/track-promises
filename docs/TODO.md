...
- [x] Add tenant-aware locale foundations: supported locale config, request resolution, locale-prefixed URLs, and multilingual font coverage.
- [x] Enforce tenant locale rules so English stays enabled and each tenant primary locale is validated against its supported locale list.
- [x] Add a shared header language switcher that keeps users on the same page while changing locale.
- [x] Translate the first template-heavy pages into Tamil: login, signup, account, and admin page shells.
- [x] Translate fixed-copy shared public components for timeline and promise pages.
...
- [x] Make Kubernetes deploy tasks rerun migration jobs and wait for them before rolling web and API workloads.
- [x] Guard local and k8s DB utility tasks against placeholder `DATABASE_URL` values so seed and migration commands fail with actionable configuration errors.
- [x] Retry transient GHCR image push failures during production deploy tasks instead of aborting the whole rollout on the first broken pipe.
...
- [x] Remove process-local auth verification tokens, guest vote/event storage, and timeline score caching from the runtime path.
- [ ] Add advanced filters for election year, timeline, alliance, jurisdiction, category, status, person/party, and source.
- [x] Externalize tenant content and domain records into localized content models.
- [x] Translate the remaining page-specific public shells on timeline overview and promise detail routes.
- [x] Load locale-specific timeline overview sidecar files with fallback to shared defaults.
- [x] Add localized tenant, timeline, and selected promise detail slugs with canonical rewrite support.
...
- [x] Make account/session verification state refresh from persisted users while demo-user auth remains transitional.
- [ ] Replace in-memory demo users with real DB-backed authentication.
- [x] Auto-assign `state: "unverified"` on signup; transition to `verified` after email or phone confirmation.
- [ ] Route suspicious or unverified signups to the moderation queue.
- [x] Add signup link in site header / login page.
- [x] Add verified-email smart-link password reset flow from the login page.
- [x] Add community-attestation self-requests so users can submit city, address, and notes, then two local verified witnesses can promote the account through the existing moderation state model.
- [x] Allow users to add, change, and remove secondary mobile numbers without breaking primary phone-sign-in accounts.
- [x] Organize account page actions into collapsible sections so one-time verification and maintenance tasks do not crowd the main profile view.
- [x] Surface the live community-score total and promotion-threshold status on the account page.
- [x] Show per-vote community-score points in the account vote history when vote activity contributes to the current scoring window.
- [x] Add a point-history section on the account page that shows current score contributions and the full rewarded-action trail.
- [x] Document the community-attestation request, witness, and auto-promotion workflow under `manuals/` for operator reference.
