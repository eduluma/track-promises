...
const SCORE_REFRESH_INTERVAL_MS = 60 * 60 * 1000;
const FORMULA_VERSION = 1;

type TimelineScoreStore = {
    records: Map<string, TimelineScoreProjection>;
};

const globalForTimelineScores = globalThis as typeof globalThis & {
    __trackPromisesTimelineScoreStore?: TimelineScoreStore;
};

function getScoreKey(tenantId: string, timelineSlug: string) {
...
export function getTimelineScoreStore() {
    if (!globalForTimelineScores.__trackPromisesTimelineScoreStore) {
        globalForTimelineScores.__trackPromisesTimelineScoreStore = {
            records: new Map()
        };
    }
...
export function getTimelineScoreProjection({
    ...
    if (!existing) {
        return upsertTimelineScoreProjection({ tenantId, timelineSlug, now });
    }

    const lastCalculatedAt = new Date(existing.calculatedAt).getTime();

    if (now.getTime() - lastCalculatedAt > SCORE_REFRESH_INTERVAL_MS) {
        return upsertTimelineScoreProjection({ tenantId, timelineSlug, now });
    }
