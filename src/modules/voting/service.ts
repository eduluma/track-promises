...
import { checkAndAutoPromote, getVoteCastScoreWeight, recordScoreEvent } from "@/modules/moderation/community-score";
...
export async function castVote({ tenantId, promiseId, user, value, now = new Date() }: CastVoteInput) {
  ...
  if (user && user.role !== "guest") {
    const voteCastScoreWeight = getVoteCastScoreWeight(tenantId);

    if (voteCastScoreWeight > 0) {
      await recordScoreEvent({
        userId,
        tenantId,
        eventType: "vote_cast",
        delta: voteCastScoreWeight,
        referenceId: `vote_cast:${tenantId}:${promiseId}:${userId}:${timestamp}:${eventRecord.eventType}`,
        metadata: {
          promiseId,
          voteValue: value,
          voteEventType: eventRecord.eventType
        },
        now
      });
      await checkAndAutoPromote(userId, tenantId);
    }
  }
