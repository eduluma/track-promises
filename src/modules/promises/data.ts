...
export type RecentElectionOverview = {
  ...
  winnerTitle: string;
  winnerTitleTranslations?: LocalizedText;
  winnerName: string;
  winnerParty: string;
...
export type PromiseRecord = {
  ...
  timelineSlug: string;
  routeSlug?: string;
  routeSlugTranslations?: LocalizedText;
  title: string;
    ...
    alliance.promises.map((promise) => {
      ...
      return {
        ...
        sources: (promise.sources ?? []).map((source, index) => ({
          ...
          excerptTranslations: source.excerptTranslations,
...
export function localizePromiseSource(source: PromiseSource, locale?: SupportedLocale | null): PromiseSource {
  if (!locale) {
    return source;
  }
  ...
  return {
    ...source,
    excerpt: resolveLocalizedText(source.excerpt, source.excerptTranslations, locale)
  };
}

export function localizeRecentElectionOverview(
  overview: RecentElectionOverview | null,
  ...
  return {
    ...overview,
    summary: resolveLocalizedText(overview.summary, overview.summaryTranslations, locale),
    winnerTitle: resolveLocalizedText(overview.winnerTitle, overview.winnerTitleTranslations, locale)
  };
}

export function localizePromiseRecord(promise: PromiseRecord, locale?: SupportedLocale | null): PromiseRecord {
  if (!locale) {
    return promise;
  }
  ...
  return {
    ...promise,
    routeSlug: resolveLocalizedText(promise.routeSlug ?? promise.id, promise.routeSlugTranslations, locale),
    title: resolveLocalizedText(promise.title, promise.titleTranslations, locale),
    description: resolveLocalizedText(promise.description, promise.descriptionTranslations, locale),
    sources: promise.sources.map((source) => localizePromiseSource(source, locale))
  };
}

export function getLocalizedPromiseRouteSlug(
  tenantId: string,
  promiseId: string,
  locale?: SupportedLocale | null,
  timelineSlug?: string | null
) {
  const promise = promiseRecords.find(
    (record) => record.tenantId === tenantId && record.id === promiseId && (timelineSlug ? record.timelineSlug === timelineSlug : true)
  );
  ...
  return resolveLocalizedText(promise.routeSlug ?? promise.id, promise.routeSlugTranslations, locale ?? null);
}

export function resolvePromiseIdFromRouteSlug(tenantId: string, timelineSlug: string, routeSlug: string) {
  return promiseRecords.find(
    (promise) =>
      promise.tenantId === tenantId &&
      promise.timelineSlug === timelineSlug &&
      (promise.id === routeSlug || promise.routeSlug === routeSlug || Object.values(promise.routeSlugTranslations ?? {}).includes(routeSlug))
  )?.id ?? null;
}

export function resolvePromiseIdFromTenantRouteSlug(tenantId: string, routeSlug: string) {
  return promiseRecords.find(
    (promise) =>
      promise.tenantId === tenantId &&
      (promise.id === routeSlug || promise.routeSlug === routeSlug || Object.values(promise.routeSlugTranslations ?? {}).includes(routeSlug))
...
export const promiseRecords: PromiseRecord[] = [
  ...
  },
  {
    ...
    tenantId: "tenant-tamilnadu",
    timelineSlug: "demo",
    routeSlug: "promise-school-meals",
