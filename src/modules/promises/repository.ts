import { getRecentElectionOverview, localizePromiseRecord, localizeRecentElectionOverview, promiseRecords, type PromiseDeliveryPlan, type PromiseSource } from "@/modules/promises/data";
...
import type { LocalizedText } from "@/modules/i18n/localized-content";
...
export async function listPromisesForTenant(tenantId: string, filters: PromiseFilters = {}) {
  ...
  return filtered.map((promise) => {
    ...
    const currentVote = filters.userId
      ...
      : null;
    const localizedPromise = localizePromiseRecord(promise, filters.locale ?? null);
...
export function getPromiseById(tenantId: string, promiseId: string, timelineSlug?: string | null, locale?: SupportedLocale | null) {
  const promise =
    promiseRecords.find(
      (promise) =>
  ...
  return promise ? localizePromiseRecord(promise, locale ?? null) : null;
}

export function getRecentElectionOverviewForTimeline(tenantId: string, timelineSlug: string, locale?: SupportedLocale | null) {
  return localizeRecentElectionOverview(getRecentElectionOverview(tenantId, timelineSlug), locale ?? null);
}
...
export async function createPromise(input: CreatePromiseInput) {
  ...
  const promise = {
    ...
    routeSlug: id,
    routeSlugTranslations: input.routeSlugTranslations,
    title: input.title,
