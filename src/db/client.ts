...
function getDatabaseUrlError(connectionString: string): string | null {
  if (!connectionString) {
    return "DATABASE_URL is required to create a database client.";
  }

  const normalizedConnectionString = connectionString.trim();

  if (!normalizedConnectionString || normalizedConnectionString === "CHANGE_ME") {
    return "DATABASE_URL is still set to a placeholder value. Update your env file with a real Postgres connection string before running DB commands.";
  }

  try {
    new URL(normalizedConnectionString);
  } catch {
...
export function createDbClient(connectionString: string = process.env.DATABASE_URL ?? "") {
  const connectionStringError = getDatabaseUrlError(connectionString);
