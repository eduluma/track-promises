...
function formatVoteCount(count: number, singularLabel: string, pluralLabel: string) {
    ...
    return (
        <div className={`min-w-0 rounded-2xl border ${palette.border} ${palette.background} ${paddingClass}`}>
            ...
            <div className="mt-2 flex items-end justify-between gap-3">
                ...
                <p className="pb-1 text-xs text-ink/50">{formatVoteCount(totalVotes, voteSingularLabel, votePluralLabel)}</p>
            </div>
                ...
                {displayedBreakdownValues.map((value) => {
                    return (
                        <div key={value} className={`flex items-center justify-between gap-3 bg-sand/45 ${rowClass}`}>
                            <span className="font-medium text-ink/80">{voteLabels[value]}</span>
                            <span className="shrink-0 tabular-nums text-ink/55">{formatVoteCount(counts[value], voteSingularLabel, votePluralLabel)}</span>
                        </div>
                    );
                })}
            </div>
